...
public class TrackingSDK: NSObject {
  ...
  public func requestAuthorization(completed: Handler?) {
    ...
    ATTrackingManager.requestTrackingAuthorization { status in
      switch status {
      case .authorized:
        LogManager.show(log: .tracking, "Enable! \(ASIdentifierManager.shared().advertisingIdentifier)")
        Analytics.setAnalyticsCollectionEnabled(true)
        LogEventManager.shared.log(event: .agreeTracking)
      default:
        LogManager.show(log: .tracking, "Disable!")
...
extension TrackingSDK: PurchaseRevenueDataSource, PurchaseRevenueDelegate {
  public func didReceivePurchaseRevenueValidationInfo(_ validationInfo: [AnyHashable : Any]?,
    ...
    LogManager.show(log: .tracking, "PurchaseRevenueDelegate: \(String(describing: validationInfo))")
    LogManager.show(log: .tracking, "PurchaseRevenueDelegate: \(String(describing: error))")
  }
...
extension TrackingSDK: AppsFlyerLibDelegate {
  public func onConversionDataSuccess(_ installData: [AnyHashable: Any]) {
    ...
    if status == "Non-organic" {
      ...
      if let sourceID = installData["media_source"],
        ...
        LogManager.show(log: .tracking, "This is a Non-organic install. Media source: \(sourceID)  Campaign: \(campaign)")
      }
    } else {
  ...
  public func onConversionDataFail(_ error: Error) {
    ...
    LogManager.show(log: .tracking, "Error: \(error)!")
  }
}
  ...
  @objc private func sendLaunch() {
    AppsFlyerLib.shared().start(completionHandler: { (dictionary, error) in
      guard error == nil else {
        LogManager.show(log: .tracking, "\(String(describing: error))!")
        LogEventManager.shared.log(event: .noConnectAppsFlyer)
        return
      }
      LogManager.show(log: .tracking, "\(String(describing: dictionary))!")
