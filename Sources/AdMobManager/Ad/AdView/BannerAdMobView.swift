...
open class BannerAdMobView: UIView {
  ...
  public func load(placement: String,
    ...
    case false:
      LogManager.show(log: .ad, "[BannerAd] Ads are not allowed to show! (\(placement))")
      errored()
      return
    case true:
      break
...
extension BannerAdMobView: GADBannerViewDelegate {
  public func bannerView(_ bannerView: GADBannerView,
    ...
    if let placement {
      LogManager.show(log: .ad, "[BannerAd] Load fail (\(placement)) - \(String(describing: error))!")
  ...
  public func bannerViewDidReceiveAd(_ bannerView: GADBannerView) {
    if let placement {
      LogManager.show(log: .ad, "[BannerAd] Did load! (\(placement))")
...
extension BannerAdMobView {
  ...
  private func load() {
    ...
    guard let adUnitID = adUnitID else {
      LogManager.show(log: .ad, "[BannerAd] Failed to load - not initialized yet! Please install ID.")
      return
    }
    
    if let placement {
      LogManager.show(log: .ad, "[BannerAd] Start load! (\(placement))")
    }
    self.state = .loading
