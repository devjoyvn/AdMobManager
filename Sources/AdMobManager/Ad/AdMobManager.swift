...
public class AdMobManager {
  ...
  public func register(remoteKey: String,
    ...
    if isPremium {
      LogManager.show(log: .config, "Premium!")
      self.state = .reject
    }
    guard self.remoteKey == nil else {
      return
    }
    LogManager.show(log: .config, "Register!")
  ...
  public func status(type: AdType, placement: String) -> Bool? {
    guard !isPremium else {
      LogManager.show(log: .ad, "Premium!")
      return nil
    }
    guard let adMobConfig else {
      LogManager.show(log: .ad, "Not yet registered!")
      return nil
    }
    ...
    guard state == .allow else {
      LogManager.show(log: .ad, "Can't Request Ads!")
      return nil
    }
    guard let adConfig = getAd(type: type, placement: placement) as? AdConfigProtocol else {
      LogManager.show(log: .ad, "Ads don't exist! (\(placement))")
      return nil
    }
    if AutoRelease.shared.isRelease == false, adMobConfig.isAuto != false, adConfig.isAuto == true {
      return false
    }
  ...
  public func load(type: Reuse,
    ...
    case false:
      LogManager.show(log: .ad, "Ads are not allowed to show! (\(placement))")
      fail?()
      return
    case true:
      break
    ...
    case .splash:
      guard let splash = adConfig as? Splash else {
        LogManager.show(log: .ad, "Format conversion error! (\(placement))")
        fail?()
        return
      }
  ...
  public func isReady(type: Reuse, placement: String) -> Bool {
    ...
    guard let ad = listReuseAd[adConfig.name] else {
      LogManager.show(log: .ad, "Ads do not exist! (\(placement))")
      return false
    }
    guard !checkIsPresent() else {
      LogManager.show(log: .ad, "Ads display failure - other ads is showing! (\(placement))")
      return false
    }
    guard checkFrequency(adConfig: adConfig, ad: ad) else {
      LogManager.show(log: .ad, "Ads hasn't been displayed yet! (\(placement))")
      return false
    }
    return true
  }
  ...
  public func preloadNative(placement: String,
    ...
    guard native.isPreload == true else {
      LogManager.show(log: .ad, "Ads are not preloaded! (\(placement))")
      fail?()
      return
    }
  ...
  public func isTestMode(nativePlacement: String) -> Bool? {
    ...
    case false:
      LogManager.show(log: .ad, "Ads are not allowed to show! (\(nativePlacement))")
      return nil
    case true:
      break
    default:
    ...
    guard let native = getAd(type: .onceUsed(.native), placement: nativePlacement) as? Native else {
      LogManager.show(log: .ad, "Ads don't exist! (\(nativePlacement))")
      return nil
    }
    guard native.isPreload == true else {
      LogManager.show(log: .ad, "Ads are not preloaded! (\(nativePlacement))")
      return nil
    }
  ...
  public func requestConsentUpdate() {
    ...
    UMPConsentForm.presentPrivacyOptionsForm(from: topVC) { [weak self] formError in
      ...
      if let formError {
        LogManager.show(log: .cmp, "Form error - \(formError.localizedDescription)!")
        return
      }
...
extension AdMobManager {
  ...
  private func decoding(adMobData: Data) {
    guard let adMobConfig = try? JSONDecoder().decode(AdMobConfig.self, from: adMobData) else {
      LogManager.show(log: .config, "Invalid (AdMobConfig) format!")
      return
    }
  ...
  private func decoding(consentData: Data) {
    guard let consentConfig = try? JSONDecoder().decode(ConsentConfig.self, from: consentData) else {
      LogManager.show(log: .config, "Invalid (ConsentConfig) format!")
      return
    }
  ...
  private func fetchRemote() {
    ...
    LogManager.show(log: .config, "[Remote config] Start load!")
    ...
    remoteConfig.fetch(withExpirationDuration: 0) { [weak self] _, error in
      ...
      LogManager.show(log: .config, "[Remote config] Success!")
  ...
  private func errorRemote() {
    guard adMobConfig == nil else {
      if didRemoteTimeout {
        LogManager.show(log: .config, "[Remote config] First load error with timeout!")
        LogEventManager.shared.log(event: .remoteConfigErrorWithTimeout)
      }
      return
    }
    LogManager.show(log: .config, "[Remote config] First load error!")
  ...
  private func timeoutRemote() {
    ...
    LogManager.show(log: .config, "[Remote config] First load timeout!")
  ...
  private func checkConsent() {
    LogManager.show(log: .cmp, "Check consent!")
    LogEventManager.shared.log(event: .cmpCheckConsent)
    guard !isPremium else {
      LogManager.show(log: .cmp, "Not request consent!")
    ...
    LogManager.show(log: .cmp, "Request consent!")
    ...
    UMPConsentInformation.sharedInstance.requestConsentInfoUpdate(with: parameters) { [weak self] requestConsentError in
      ...
      if let requestConsentError {
        LogManager.show(log: .cmp, "Request consent error - \(requestConsentError.localizedDescription)!")
      ...
      UMPConsentForm.loadAndPresentIfRequired(from: topVC) { [weak self] loadAndPresentError in
        ...
        if let loadAndPresentError {
          LogManager.show(log: .cmp, "Load and present error - \(loadAndPresentError.localizedDescription)!")
        ...
        guard isGDPR() else {
          LogManager.show(log: .cmp, "Auto agree consent GDPR!")
        ...
        let canShowAds = canShowAds()
        if canShowAds {
          LogManager.show(log: .cmp, "Agree consent!")
          LogEventManager.shared.log(event: .cmpAgreeConsent)
          self.startGoogleMobileAdsSDK()
        } else {
          LogManager.show(log: .cmp, "Reject consent!")
    ...
    if canShowAds() {
      LogManager.show(log: .cmp, "Auto agree consent!")
