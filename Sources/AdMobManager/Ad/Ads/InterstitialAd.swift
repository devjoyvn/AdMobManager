...
class InterstitialAd: NSObject, AdProtocol {
  ...
  func show(placement: String,
    ...
    guard !isShowing else {
      LogManager.show(log: .ad, "[InterstitialAd] Display failure - ads are being displayed! (\(placement))")
      didFail?()
      return
    }
    LogEventManager.shared.log(event: .adShowRequest(placement))
    guard isReady() else {
      LogManager.show(log: .ad, "[InterstitialAd] Display failure - not ready to show! (\(placement))")
      didFail?()
      return
    }
    guard wasLoadTimeGreaterThanInterval() else {
      LogManager.show(log: .ad, "[InterstitialAd] Display failure - Load time is less than interval! (\(placement))")
      didFail?()
      return
    }
    LogEventManager.shared.log(event: .adShowReady(placement))
    LogManager.show(log: .ad, "[InterstitialAd] Requested to show! (\(placement))")
...
extension InterstitialAd: GADFullScreenContentDelegate {
  func ad(_ ad: GADFullScreenPresentingAd,
    ...
    if let placement {
      LogManager.show(log: .ad, "[InterstitialAd] Did fail to show content! (\(placement))")
  ...
  func adWillPresentFullScreenContent(_ ad: GADFullScreenPresentingAd) {
    if let placement {
      LogManager.show(log: .ad, "[InterstitialAd] Will display! (\(placement))")
  ...
  func adDidDismissFullScreenContent(_ ad: GADFullScreenPresentingAd) {
    if let placement {
      LogManager.show(log: .ad, "[InterstitialAd] Did hide! (\(placement))")
...
extension InterstitialAd {
  ...
  private func load() {
    ...
    guard let adUnitID = adUnitID else {
      LogManager.show(log: .ad, "[InterstitialAd] Failed to load - not initialized yet! Please install ID.")
      return
    }
    ...
    DispatchQueue.main.async { [weak self] in
      ...
      if let name {
        LogManager.show(log: .ad, "[InterstitialAd] Start load! (\(name))")
      ...
      GADInterstitialAd.load(
        ...
        guard error == nil, let ad = ad else {
          ...
          let delaySec = 5.0
          if let name {
            LogManager.show(log: .ad, "[InterstitialAd] Did fail to load. Reload after \(delaySec)s! (\(name)) - (\(String(describing: error)))")
        ...
        if let name {
          LogManager.show(log: .ad, "[InterstitialAd] Did load! (\(name))")
